 ...
 * failed to reply because the OpenAI account had no credit (recurring
 * "insufficient_quota" outage). Finds SDR conversations whose newest
 * message is still role=USER (customer waiting, no AI reply ever sent) and
 * replays processSdrResponse for each, now that credit exists.
  ...
  for (const provider of providers) {
    ...
    const conversations = await prisma.whatsappConversation.findMany({
      where: {
        ...
        humanTakeover: false,
        lastMessageAt: { gte: new Date(Date.now() - 96 * 60 * 60 * 1000) },
