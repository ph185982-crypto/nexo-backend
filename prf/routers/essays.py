...
async def list_themes(
    ...
    """List available essay themes."""
    themes = await repo._fetch(
        "SELECT id, title, description, context_text, subject_area, source, year "
        "FROM essay_themes WHERE is_active = TRUE ORDER BY created_at DESC"
    )
    ...
    row = await repo._fetchrow(
        """INSERT INTO essays
           (user_id, theme_id, theme_title, input_type, original_text,
            ...
            final_score, diagnosis, feedback_text)
...
async def essay_history(
    ...
    essays = await repo._fetch(
        """SELECT id, theme_title, input_type, total_lines,
                  nc_score, ne_count, penalty, final_score, created_at
