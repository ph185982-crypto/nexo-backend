...
def _build_units(topics: list[dict]) -> list[dict]:
    """Agrupamento de tópicos em unidades — implementação em audio_pipeline,
    para o cron e a manutenção usarem exatamente o mesmo plano de aulas."""
    from prf.services.audio_pipeline import build_units
    return build_units(topics)
...
async def generate_podcast_episode(
    ...
    Uma parte por chamada: são dez blocos de roteiro via LLM e o tempo da
    função não comporta mais. Chame em loop até `remaining` zerar — ou deixe
    a tarefa agendada (/cron/audio) fazer isso sozinha, todo dia.
    """
    _require_admin(x_maintenance_token)
    from prf.services.audio_pipeline import gerar_proxima_aula
    return await gerar_proxima_aula(repo, unit_slug)
...
async def generate_podcast_drill(
    ...
    """Gera o DRILL da volta para a próxima aula que ainda não tem."""
    _require_admin(x_maintenance_token)
    from prf.services.audio_pipeline import gerar_proximo_drill
