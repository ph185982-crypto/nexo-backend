...
# IMPORTANT: specific routes MUST come before /{product_id}
...
async def trigger_scan(req: ScanRequest, background_tasks: BackgroundTasks, user=Depends(get_current_user)):
    ...
    background_tasks.add_task(run_full_scan, scan_id, req.keywords, req.min_markup)
    return {"scan_id": scan_id, "status": "running"}


async def run_full_scan(scan_id: str, keywords: List[str], min_markup: float):
    d = Database()
    try:
        ...
        br = await ShopeeScraper().search_products(keywords)
