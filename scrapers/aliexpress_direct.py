...
class AliExpressDirectScraper:
    ...
    async def _direct_search(self, keyword: str, max_results: int) -> List[Dict]:
        ...
        try:
            ...
            logger.warning(f"Scraping direto falhou ({e})")
            return []

    def _parse_html(self, html: str, max_results: int) -> List[Dict]:
        results = []
        pattern = r'"mods":\{"itemList":\{"content":(\[.*?\])'
    ...
    def _parse_item(self, item: dict) -> Dict:
        try:
            ...
            if isinstance(img, dict):
                ...
                img_url = str(img or "")
            orders = item.get("trade", {}).get("realTrade", "0")
            orders = int(re.sub(r'[^0-9]', '', str(orders))) if orders else 0
            rating_info = item.get("evaluation", {})
            rating = float(rating_info.get("starRating", 4.5)) if isinstance(rating_info, dict) else 4.5
            return {
                "platform": "aliexpress", "product_id": pid, "title": title,
                "price_usd": price_usd, "images": [img_url] if img_url else [],
                "image_url": img_url, "rating": rating, "orders_count": orders,
                "product_url": f"https://www.aliexpress.com/item/{pid}.html",
                "supplier_name": "", "category": "Outros",
            }
        except Exception:
            return {}
    ...
    def _norm(self, i) -> Dict:
        ...
        imgs = i.get("images", [i.get("image", "")]); img_url = imgs[0] if imgs else ""
        return {
            "platform": "aliexpress", "product_id": str(i.get("id", i.get("itemId", ""))),
            "title": i.get("title", i.get("name", "Unknown")), "price_usd": pusd,
            "images": imgs, "image_url": img_url,
    ...
    def _valid(self, i) -> bool:
        pr = i.get("price", {}); v = pr.get("min", pr.get("value", 0)) if isinstance(pr, dict) else pr
